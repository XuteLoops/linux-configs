...
| `70-limine-snapshot-boot.sh` | `limine-mkinitcpio-hook`, `limine-snapper-sync` (AUR)                                                                                                                                                | Wires snapper snapshots into the Limine boot menu, including detecting and adding the correct overlay hook (`btrfs-overlayfs` or `sd-btrfs-overlayfs`, based on whether mkinitcpio uses `udev` or `systemd` hooks) so a booted snapshot is actually writable, not a broken read-only boot. Requires `15-snapper-setup.sh` to have run first. **Limine only** — detects and cleanly skips GRUB rather than guessing at unsupported territory. Stops short of auto-editing `limine.conf`'s boot entries themselves; if no injection marker is found, tells you exactly what line to add and where. |
